...
class Vector2 {
    ...
    add(vect) {
        if (!(vect instanceof Vector2)) {
            console.error("Argument must be instance of Vector2 class"); return;
        }
    ...
    multiplyScalar(value) {
        if (typeof value != "number") { console.error("Srgument must be number"); return; }
    ...
    belongsToPolygon(poly) {
        //Data validation
        if (!Array.isArray(poly)) { console.error("Argument must be array"); return; }
        for (let i = 0; i < poly.length; i++){
            if (!(poly[i] instanceof Vector2)) { console.error("Array must contain Vector2 objects"); return; }
        }
...
function mapValue(value, in_min, in_max, out_min, out_max) {
    for (let i = 0; i < arguments.length; i++){
        if (typeof arguments[i] != "number") { console.error("All argument must be numbers"); return; }
    }
...
function randomInt(min, max) {
    if (typeof min != "number" || typeof max != "number") {
        console.error("All argument must be numbers"); return;
    }
...
let MOUSEPRESSED = undefined;
/**
 * Enables mousePress events and stores boolean in a global MOUSEPRESSED when mouse is pressed
 */
function enableMousePressEvent() {
    MOUSEPRESSED = false;
    document.body.onmousedown = function () { MOUSEPRESSED = true; }
    document.body.onmouseup = function () { MOUSEPRESSED = false; }
}
...
class CookiesHelper {
    ...
    getByName(name) {
        ...
        //Trying to parse
        try { toRetun = JSON.parse(toRetun); return toRetun; }
        catch (e) { return toRetun; }
    }
...
/**
 * Creates 2D Array object with specyfied amount of rows and collumns. By default is field with 0.
 * @param {Number} row
 * @param {Number} coll
 * @returns {Array}
 */
function create2DArray(row, coll) {
    //Validation
    if (typeof row != "number" || typeof coll != "number") {
        console.error("Rows and collumn must be numbers");
        return;
    }
