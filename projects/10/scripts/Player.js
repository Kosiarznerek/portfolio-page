class Player {
    ...
    _isAbleToMove(direction) {
        ...
        if (USER_INTERFACE.GAME_SETTINGS.DUNGEON_EDITOR_ENABLED) {
            if (row == 0 || row == 29) { return false; }
            if (coll == 0 || coll == 59) { return false; }
            return true;
        }
    ...
    applyMovements() {
        ...
        if (this._isAbleToMove(direction)) {
            ...
            if (direction.x > 0) { this.lookingDirection.positionSet(1, 0); }
            if (direction.x < 0) { this.lookingDirection.positionSet(-1, 0); }
            if (direction.y > 0) { this.lookingDirection.positionSet(0, 1); }
            if (direction.y < 0) { this.lookingDirection.positionSet(0, -1); }
        }
    }
    ...
    updateShooting() {
        ...
        function notToClose() {
            ...
            if (LEVEL_OBJECTS.map[row][coll] == "XX") { return true }
            else return false;
        }
    ...
    collectItems() {
        ...
        if (grabbedItem) { this.grabItem(grabbedItem) }
    }
    ...
    grabItem(item, index) {
        ...
        switch (item.className) {
            case "02": this.scores += 200; break;
            case "07": this.backpack.bombs.push(item); break;
            case "08": this.backpack.food.push(item); break;
            case "11": this.backpack.keys.push(item); break;
            case "13": console.info("NEXT LEVEL"); break;
        }
        this.updateStats();
