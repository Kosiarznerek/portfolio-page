...
window.onload = function () {
    ...
    PreloadGraphics(IMAGES, function () {
        ...
        setInterval(function () { animate(ctx, width, height) }, 1000 / 60);
    })
...
function animate(ctx, CANVAS_WIDTH, CANVAS_HEIGHT) {
    ...
    if (x <= 320) { x = 320; } if (x >= 1600) { x = 1600; }
    if (y <= 160) { y = 160; } if (y >= 800) { y = 800; }
    ctx.translate(-x, -y);
    ...
    if (!USER_INTERFACE.GAME_SETTINGS.MENU_ENABLED || USER_INTERFACE.GAME_SETTINGS.DUNGEON_EDITOR_ENABLED) {
        //PLAYER MOVEMENTS
        if (KEYPRESSED["W"]) { LEVEL_OBJECTS.player.move.up = true; }
        else { LEVEL_OBJECTS.player.move.up = false; }
        if (KEYPRESSED["S"]) { LEVEL_OBJECTS.player.move.down = true; }
        else { LEVEL_OBJECTS.player.move.down = false; }
        if (KEYPRESSED["A"]) { LEVEL_OBJECTS.player.move.left = true; }
        else { LEVEL_OBJECTS.player.move.left = false; }
        if (KEYPRESSED["D"]) { LEVEL_OBJECTS.player.move.right = true; }
        else { LEVEL_OBJECTS.player.move.right = false; }
        ...
        if (!USER_INTERFACE.GAME_SETTINGS.DUNGEON_EDITOR_ENABLED) {
            //PLAYER SHOOTING
            if (KEYPRESSED["SPACE"]) { LEVEL_OBJECTS.player.shooting = true; }
            else { LEVEL_OBJECTS.player.shooting = false; }
            LEVEL_OBJECTS.player.updateShooting();

            //PLAYER EATS FOOD
            if (KEYPRESSED["F"]) { LEVEL_OBJECTS.player.eatFood(); }

            //USING BOMB
            if (KEYPRESSED["B"]) { LEVEL_OBJECTS.player.useBomb(); }
