...
class UserInterface {
    ...
    gameMenuControling() {
        if (this.GAME_SETTINGS.MENU_ENABLED) {
            ...
            if (!this.GAME_SETTINGS.DUNGEON_EDITOR_ENABLED) {
                //SELECTING LEVEL
                if (KEYPRESSED["1"]) { this._changeLevel(1); }
                if (KEYPRESSED["2"]) { this._changeLevel(2); }
                if (KEYPRESSED["M"]) { this._changeGameMode() }
                if (KEYPRESSED["B"]) {
                    switch (this.GAME_SETTINGS.GAME_MODE) {
                        case "ONE PLAYER": this._beginGame(); break;
                        case "DUNGEON EDITOR": this._enableDungeonEditor(); break;
                    }
                }
            }
            ...
            //ONLY FOR DUNGEON EDITOR
                ...
                if (KEYPRESSED["ARROWLEFT"]) { this._changeLevel("down") }
                if (KEYPRESSED["ARROWRIGHT"]) { this._changeLevel("up") }
                if (KEYPRESSED["Q"]) { this._exitDungeonEditor(); }
                //EDITING MAP
                if (KEYPRESSED["O"]) { this._editGameMap("XX") };
                if (KEYPRESSED["K"]) { this._editGameMap("01") };
                if (KEYPRESSED["M"]) { this._editGameMap("02") };
                if (KEYPRESSED["I"]) { this._editGameMap("03") };
                if (KEYPRESSED["J"]) { this._editGameMap("05") };
                if (KEYPRESSED["N"]) { this._editGameMap("06") };
                if (KEYPRESSED["U"]) { this._editGameMap("07") };
                if (KEYPRESSED["H"]) { this._editGameMap("08") };
                if (KEYPRESSED["B"]) { this._editGameMap("09") };
                if (KEYPRESSED["Y"]) { this._editGameMap("10") };
                if (KEYPRESSED["G"]) { this._editGameMap("11") };
                if (KEYPRESSED["V"]) { this._editGameMap("12") };
            }
        }
    }
    ...
    _editGameMap(itemID) {
        ...
        switch (levelToEdit) {
            case 1: mapToEdit = LEVEL01; break;
            case 2: mapToEdit = LEVEL02; break;
        }
    ...
    _changeLevel(level) {
        if (typeof level == "number" && Number.isInteger(level)) {
            ...
            switch (level.toUpperCase()) {
                case "UP": newCurrent++; break;
                case "DOWN": newCurrent--; break;
                default: console.error("UNRECOGNIZED LEVEL IN USER INTERFACE " + level); break;
            }
            //validation
            if (newCurrent < 1) { newCurrent = this.GAME_SETTINGS.TOTAL_AMOUNT_OF_LEVELS };
            if (newCurrent > this.GAME_SETTINGS.TOTAL_AMOUNT_OF_LEVELS) { newCurrent = 1; };
