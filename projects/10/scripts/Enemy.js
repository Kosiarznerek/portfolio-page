class Enemy {
    ...
    constructor(x, y, className) {
        ...
        switch (this.className) {
            case "03": this._image = IMAGES.img03; this.lifes = 2; this.static = false; break;
            case "05": this._image = IMAGES.img05; this.lifes = 1; this.static = false; break;
            case "06": this._image = IMAGES.img06; this.lifes = 3; this.static = false; break;
            case "09": this._image = IMAGES.img09; this.lifes = 4; this.static = true; break;
            case "12": this.static = true; this.lifes = Infinity; this._image = IMAGES.img12; break;
            default: console.error("ENEMY CLASS NAME UNRECOGNIZED: " + this.className); break;
        }
    }
    ...
    updateLifes() {
        switch (this.lifes) {
            case 1: this._image = IMAGES.img05; this.className = "05"; this.static = false; break;
            case 2: this._image = IMAGES.img03; this.className = "03"; this.static = false;  break;
            case 3: this._image = IMAGES.img06; this.className = "06"; this.static = false;  break;
            case 4: this._image = IMAGES.img09; this.className = "09"; this.static = true; break;
            case 0:
    ...
    followTarget(targetPosition) {
        for (let dir in this.move) { this.move[dir] = false; }
        ...
        if (targetPosition.distanceTo(this.position) <= this.FOV && !this.dead && !this.static) {
            ...
            if (direction.x > 0) { this.move.right = true; }
            if (direction.x < 0) { this.move.left = true; }
            if (direction.y > 0) { this.move.down = true; }
            if (direction.y < 0) { this.move.up = true; }
        }
    ...
    applyMovements() {
        ...
        if (this._isAbleToMove(direction) && !this.dead) {
            ...
            let inter = setInterval(function () {
                ...
                that.position.add(v);
                counter++;
                if (counter == steps) { clearInterval(inter); }
            }, 15);

            //UPDATING STOWATCHES
            if (direction.x < 0) { this._movesStopwatch.left.start(); }
            if (direction.x > 0) { this._movesStopwatch.right.start(); }
            if (direction.y < 0) { this._movesStopwatch.up.start(); }
            if (direction.y > 0) { this._movesStopwatch.down.start(); }
        }
    }
