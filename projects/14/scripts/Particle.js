class Particle {
    ...
    enableFlickering(setsOfCollors, time, _delay = 0) {
        ...
        setTimeout(function () {
            ...
            setInterval(function () {
                if (that.visible) { that.visible = false; }
                else { that.visible = true; }
            }, time)
        }, _delay)
    }
    ...
    isOffCanvas() {
        //Y
        if (this.position.y > CANVAS_HEIGHT) { return true }
        if (this.position.y + this.height < 0) { return true }

        //X
        if (this.position.x > CANVAS_WIDTH) { return true }
