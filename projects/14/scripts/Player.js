class Player {
    ...
    updateMovements() {
        ...
        if (this.move.left) { force.x = -1; }
        if (this.move.right) { force.x = 1; }
        if (this.move.up) { force.y = -1; }
        if (this.move.down) { force.y = 1; }
    ...
    _canMove(direction) {
        ...
        //Checking
        if (newPosition.x + 60 >= CANVAS_WIDTH) { return false }
        if (newPosition.x <= 0) { return false }
        if (newPosition.y > 300) { return false }
